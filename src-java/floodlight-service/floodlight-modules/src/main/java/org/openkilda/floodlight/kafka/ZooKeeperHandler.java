...
public class ZooKeeperHandler implements Runnable, LifeCycleObserver {
    ...
    public ZooKeeperHandler(String id, String serviceName, int apiVersion, String connectionString) {
        try {
            ZkWatchDog watchDog = ZkWatchDog.builder().id(id).serviceName(serviceName)
                    .apiVersion(apiVersion).connectionString(connectionString).build();
            watchDog.subscribe(this);

            ZkWriter zkWriter = ZkWriter.builder().id(id).serviceName(serviceName).apiVersion(apiVersion)
